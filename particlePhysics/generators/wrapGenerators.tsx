...
export function wrapAroundGenerator() {
  return {
    name: "wrap around",
    function: (particle: Tparticle, boundary: Iparallelepiped) => {
...
export function wrapBounceGenerator(coeficient: number = 1) {
  return {
    name: `bounce with coeficient ${coeficient}`,
    function: (particle: Tparticle, boundary: Iparallelepiped) => {
      let maxX = boundary.x + boundary.hw;
      let minX = boundary.x - boundary.hw;
      let maxY = boundary.y + boundary.hh;
