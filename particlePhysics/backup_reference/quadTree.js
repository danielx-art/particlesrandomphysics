const quadTree = function (boundary, capacity) {
  ...
  self.remove = function (point) {
    ...
    if (indexToRemove > -1) {
      ...
      if (self.divided) {
        if (self.northeast.remove(point)) {
          return true;
        }
        if (self.northwest.remove(point)) {
          return true;
        }
        if (self.southeast.remove(point)) {
          return true;
        }
        if (self.southwest.remove(point)) {
          return true;
        }
      }
    }
  ...
  self.count = function () {
    ...
    if (self.divided) {
      count +=
        self.northeast.count() +
        self.northwest.count() +
        self.southeast.count() +
        self.southwest.count();
    }
    return count;
  };
